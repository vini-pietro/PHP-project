...
$library = new Library(
    array_map(fn($data) => new Book($data['title'], $data['author'], $data['isbn']), loadFromFile('books.json')), // Mandatory: Use expressions (e.g., lambda functions)
...
while (true) {
    ...
    switch ($choice) {
        case '1': // Generate Book List
            // Mandatory: Demonstrate iteration (foreach)
            echo "Book List:\n";
        ...
        case '3': // Add New Book
            // Mandatory: Use data types like strings and integers
        ...
        case '7': // Search Book by ID
            ...
            $id = (int)trim(fgets(STDIN));
            $found = false; // Mandatory: Demonstrate control using flags
            foreach ($library->listBooks() as $book) {
                if ($book->getId() === $id) {
                    echo $book->getDescription() . "\n";
                    $found = true;
        ...
        case '8': // Sort Books in Ascending Order
            // Mandatory: Sort elements in an array
            usort($library->listBooks(), fn($a, $b) => strcmp($a->getTitle(), $b->getTitle()));
        ...
        case '9': // Sort Books in Descending Order
            usort($library->listBooks(), fn($a, $b) => strcmp($b->getTitle(), $a->getTitle()));
        ...
        case '10': // Exit the program
            // Mandatory: Demonstrate decision-making (exit the program)
